...
import {
  ...
  CalendarDays, IndianRupee, Tag, AlertCircle, X
} from 'lucide-react'
...
function BatchCard({ batch, onDelete, onStatusChange }) {
  ...
  return (
    <div className="card p-0 overflow-hidden">
      <div
        className="px-4 py-3 flex items-start sm:items-center gap-3 cursor-pointer hover:bg-cyan-50 transition-colors"
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-0.5">
            <span className="text-xs text-sky-600 flex items-center gap-1">
              <Tag size={10} />{batch.category}
            </span>
            <span className="text-xs text-sky-600 flex items-center gap-1">
              <CalendarDays size={10} />Due {batch.dueDate}
            </span>
            <span className="text-xs text-sky-600 flex items-center gap-1">
              <IndianRupee size={10} />{fmt(batch.amount)}/unit
            </span>
          </div>
        </div>
...
export default function MaintenanceManagement() {
  ...
  const loadConfigs  = useCallback(async () => {
    try {
      ...
      setConfigs(Array.isArray(res.data?.data) ? res.data.data : Array.isArray(res.data) ? res.data : [])
    } catch {}
  }, [])
  ...
  const openCreate = () => { setForm(BLANK); setFlatSearch(''); setModalOpen(true) }
  ...
  const handleSaveConfig   = async () => {
    try {
      const p = { ...configForm, ratePerSqFt: configForm.ratePerSqFt ? Number(configForm.ratePerSqFt) : null, amount: configForm.amount ? Number(configForm.amount) : null, lateFee: Number(configForm.lateFee) || 0 }
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-fade-in">
          ...
          <button key={key} onClick={() => setTab(key)}
            className={`px-4 py-2.5 text-xs font-semibold border-b-2 transition-colors ${
              tab === key
                ...
                : 'border-transparent text-sky-400 hover:text-sky-700'
            }`}>
            {lbl}
          </button>
        ))}
                  ...
                  <tbody>
                    {configs.map(c => (
                      <tr key={c.id} className="table-row">
                        ...
                        <td className="table-cell font-mono">{c.ratePerSqFt ? `₹${c.ratePerSqFt}` : '—'}</td>
                        ...
                        <td className="table-cell">
                          <div className="flex gap-3">
                            <button onClick={() => { setEditConfig(c); setConfigForm({ maintenanceType: c.maintenanceType, ratePerSqFt: c.ratePerSqFt ?? '', amount: c.amount ?? '', dueDate: c.dueDate ?? todayStr(), lateFee: c.lateFee ?? '', lateFeeEnabled: c.lateFeeEnabled }); setConfigModal(true) }}
                              className="text-xs text-sky-600 hover:text-sky-800">Edit</button>
                            <button onClick={async () => { if (!confirm('Delete?')) return; try { await adminAPI.deleteMaintenance(c.id); toast.success('Deleted'); loadConfigs() } catch { toast.error('Delete failed') } }}
      ...
      {/* ══ CREATE BATCH MODAL ══════════════════════════════════════
          Modal handles its own scrolling — no fixed-height children  */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)}
        title="Create Maintenance Batch" size="lg">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.title} onChange={e => patch('title', e.target.value)}
                placeholder="e.g. May 2025 — Monthly Maintenance"
          ...
          <div>
            ...
            <textarea value={form.description} onChange={e => patch('description', e.target.value)}
              placeholder="Additional notes for residents…" rows={2}
          ...
          <div className="rounded-xl border border-cyan-200 bg-cyan-50/50 p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => patch('penaltyEnabled', !form.penaltyEnabled)}
                className="text-sky-500 hover:text-sky-700 transition-colors">
                {form.penaltyEnabled
                  ? <ToggleRight size={24} className="text-sky-600" />
                  : <ToggleLeft  size={24} />}
              </button>
            </div>
            {form.penaltyEnabled && (
              <div>
                ...
                <div className="relative max-w-xs">
                  ...
                  <input type="number" value={form.penaltyAmount}
                    onChange={e => patch('penaltyAmount', e.target.value)}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {ASSIGNMENT_TYPES.map(({ value, label, desc }) => (
                <button key={value}
                  onClick={() => patch('assignmentType', value)}
                  className={`rounded-xl border p-3 text-left transition-all ${
                    form.assignmentType === value
                      ? 'border-sky-400 bg-sky-50'
                      : 'border-cyan-200 hover:border-sky-300 bg-white'
                  }`}>
          ...
          {form.assignmentType === 'BLOCK' && (
            <div>
              ...
              <input value={form.blockPrefix}
                onChange={e => patch('blockPrefix', e.target.value.toUpperCase())}
          ...
          {form.assignmentType === 'INDIVIDUAL' && (
            <div className="rounded-xl border border-cyan-200 bg-white p-3 space-y-3">
              <div className="flex items-center justify-between">
                ...
                {form.selectedFlats.length > 0 && (
                  <button onClick={() => patch('selectedFlats', [])}
                    className="text-[10px] text-sky-400 hover:text-red-500">Clear all</button>
                )}
              </div>
              <div className="relative">
                <Search size={11} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-sky-400" />
                <input type="text" placeholder="Search flat or name…"
                  ...
                  return (
                    <button key={r.id} onClick={() => toggleFlat(r.flatNumber)}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-sky-500">{r.flatNumber} · {r.squareFeet ?? '?'} sq.ft</p>
                      </div>
                    </button>
                  )
                })}
          ...
          <div className="flex gap-2 bg-sky-50 border border-sky-200 rounded-xl px-3 py-2.5">
            ...
            <p className="text-[11px] text-sky-600 leading-relaxed">
              A PENDING payment record will be created for each matched active resident.
              Residents with an existing payment for the same month are skipped automatically.
            </p>
          </div>
